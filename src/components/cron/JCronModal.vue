    ...
    <div class="card-container">
      ...
      <div class="cron-result">
        <div class="flex justify-center align-center" style="height:50px;">
          <span class="bottom">{{cron}}</span>
          <!-- <a-button size="small" @click="getCronExplain(cron)">查看最近10次运行时间</a-button> -->
        </div>
        <span>最近10次运行时间：</span>
        <div class="explainText">
          <span v-if="explainText.length">
            <p v-for="(item,index) in explainText" :key="index">{{item}}</p>
          </span>
          <span v-else style="color:rgb(221, 53, 53)">{{explainTextMessage}}</span>
        </div>
      </div>
    </div>
  </a-modal>
...
export default {
  ...
  data() {
    return {
      Util,
      api: {
        cronApi: Util.systemServer + "/web/alarm-task/time" //系统管理员列表
      },
      visible: false,
      activeKey: "1",
  ...
  computed: {
    ...
    cron() {
      let cron = `${this.secondsText || "*"} ${this.minutesText || "*"} ${this
        .hoursText || "*"} ${this.daysText || "*"} ${this.monthsText ||
        "*"} ${this.weeksText || "?"}`;
      this.getCronExplain(cron);
      return cron;
  ...
  watch: {
    ...
    cron: {
      handler(val) {
        //this.explainText = this.cronChangeDate(val);
      },
      immediate: true
    }
  },
  methods: {
    ...
    getCronExplain(cron) {
      this.$post(this.api.cronApi, { cron })
        .then(response => {
          if (response.statusCode == 200) {
            this.explainText = response.data;
          } else {
            this.explainText = [];
            this.explainTextMessage = response.statusMessage;
          }
        })
        .catch(res => {
    ...
    secondsReverseExp(seconds) {
      ...
      switch (true) {
        ...
        case val.includes("/"):
          ...
          break;
        case val.includes(",") || !isNaN(Number(val)):
          second.cronEvery = "3";
      ...
      if (!days.includes("?")) {
        switch (true) {
          ...
          case days.includes("/"):
            ...
            break;
          case days.includes(",") || !isNaN(Number(val)):
            day.cronEvery = "5";
        ...
        switch (true) {
          case weeks.includes("/"):
            ...
            break;
          case weeks.includes(",") || !isNaN(Number(val)):
            day.cronEvery = "4";
