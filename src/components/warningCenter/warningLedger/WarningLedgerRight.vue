  ...
  methods: {
    ...
    getTableOptions() {
      if (this.tabValue == "a") {
        ...
        this.table.operValue = [0, 1, 2, 3, 4];
    ...
    /**催办 批量 */
    remindToDo() {
      this.remindToDoAjax(this.table.selectedRowKeys);
    },
    /**催办 单个 */
    remindToDoSingle(record) {
      this.remindToDoAjax([record.id]);
    },
    remindToDoAjax(data) {
      let params = {
        ids: data
    ...
    operChange(checkedValues) {
      ...
      this.currentId ? this.searchList(true) : "";
    },
    /**
     * 删除
     * @param id
