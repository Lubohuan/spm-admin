...
<template>
  <div id="LoginByAdmin" class="flex">
    <div class="ad">
      <a-carousel arrows class="ad-swiper">
        ...
        <img style="width: 100%;min-width: 1200px;" src="../assets/image/login/swiper2.jpg" alt="">
        <img style="min-width: 1200px;" src="../assets/image/login/swiper.jpg" alt="">
      </a-carousel>
    </div>
          ...
          >
            ...
            <a-form-item >
              <div class="flex justify-between align-center" style="font-size: 12px">
                <a-checkbox :checked="rememberMe" @change="rememberMeChange"><span style="font-size: 12px">记住密码</span></a-checkbox>
            ...
            <a-form-item style="margin-top:24px" class="flex align-center" @click="isAccount = !isAccount" v-show="false">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        api: {
          ...
          accountLoginApi: Util.adminServer + '/admin-user/login', // 账号登录
    ...
    mounted() {
      ...
      // form数据回填
      let account = Cookie.get('username')
      console.log(Cookie.get('remember'))
      this.rememberMe = (Cookie.get('remember') == 'false' ?  false : true) || false
      let passportPwd = Cookie.get('password')
      let obj = {}
      if( this.rememberMe) {
        obj = {
          username: account,
          password: passportPwd,
        }
      }else{
        obj = {
          username: account,
    ...
    methods: {
        ...
        if(this.isAccount) {
          this.accountForm.validateFields((err, values) => {
            if (!err) {
              ...
              Cookie.set('username', values.username, { expires: 7 })
              this.loginSubmit('account', values)
            }
          });
        }else{
      ...
      loginSubmit(type, params) {
        ...
        if(type == 'account') {
          ...
          post = this.$post(_loginApi,Util.delParams(params, ['username', 'password']))
        }else{
          _loginApi = this.api.telLoginApi
          post = this.$postParams(_loginApi,Util.delParams(params, ['account', 'code']))
        }
        post.then((response) => {
          this.loading = false
          Util.processRes(response, () => {
            Cookie.set('isPlatform', 'yes')
            if(type !== 'account') {
              let accountList = response.data.passportVO.accountList
              Cookie.set('jwt', 'Bearer ' + response.data.tokenVo.accessToken, { expires: 7 })
              accountList && accountList.length && Cookie.set('sessions', JSON.stringify({passportId: accountList[0].passportId, tenantId: accountList[0].tenantId}))
              this.$store.commit('SET_CURR_TENANT', accountList[0])
            }else{
              Cookie.set('remember', this.rememberMe, { expires: 7 })
              if(this.rememberMe) {
                Cookie.set('password', params.password, { expires: 7 })
              }
              Cookie.set('jwt', 'Bearer ' + response.data.accessToken, { expires: 7 })
              Cookie.set('sessions', JSON.stringify({passportId: response.data.id}))
            }
            response.data['name'] = response.data['nickname']
            this.$store.commit('SET_LOGINMSG', response.data)

            this.getResource(response.data.id)
