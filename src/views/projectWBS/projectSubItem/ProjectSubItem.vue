...
<template>
  <div id="ProjectSubItem" class="page-box">
          ...
          <div class="search-lower">
            <div class="flex justify-between">
              <div class="table-config">
                <a-button type="primary" v-waves @click="showAddModal(false)">新增</a-button>
                <!-- <a-button
                  type="primary"
                  v-waves
                ...
                <a-button
                  ...
                  :disabled="table.data.length?true:false"
                  :title="table.data.length?'已存在分部分项，请删除后再选择模板':''"
                >选择模板</a-button>
    ...
    />
    <ModalImportant ref="ModalImportant" upLoadType="3" @searchList="searchList" />
    <CheckedTree
      :multiple="true"
      v-if="Visible"
      :isVisible="Visible"
      modalType="double"
      apiType="selectTemplate"
      title="选择模板"
...
import ModalImportant from "@/modalImportant/ModalImportant"; //导入模态框 //导入模态框
import CheckedTree from "@/select/CheckedTree"; //选择模板组件
export default {
  name: "ProjectSubItem",
  components: {
    Title, //标题
  ...
  data() {
    return {
      Util,
      api: {
        treeList: Util.wbsServer + "/web/wbs/wbs-project/tree", // 获取列表
        editList: Util.wbsServer + "/web/wbs/wbs-project/edit-many", // 编辑
        delete: Util.wbsServer + "/web/wbs/wbs-project/delete", // 删除
        exportApi: Util.wbsServer + "/web/wbs/wbs-project/export-excel", //施工区域导出excel
        addManyApi: Util.wbsServer + "/web/wbs/wbs-project/add-many" //批量关联
      },
  ...
  methods: {
    ...
    setValue(ids) {
      ...
      this.$post(this.api.addManyApi, params)
        .then(response => {
    ...
    exportExcel() {
      ...
      this.$exportFile(this.api.exportApi, params)
        .then(response => {
          ...
          a.setAttribute("download", this.curr_org.name + "分部分项.xls");
          a.click();
        })
        .catch(res => {
          console.log(res);
